...
- **Shared helpers** live in `R/` and load via `source("00_functions.R")` in scripts 03–07 (and automatically in interactive RStudio via `.Rprofile`). Do not attach `bit64` globally — use `bit64::` namespaced calls only; attaching it breaks `dplyr::filter()`. Do not use `.Renviron` for sourcing R code — reserve `.Renviron` for environment variables (e.g. `DATAVERSE_SERVER`).
